...
class Verify {
	 ...
	 * @var string
	 */
	private $_codeSet = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
	private $_codeSZ  = '0123456789';
	private $_codeSS  = '';
	private $_codeVal = 0;
	...
	public function entry($id = '') {
		// 图片宽(px)
		$this->imageL || $this->imageL = $this->length*$this->fontSize*1.2 + $this->length*$this->fontSize/2; 
		// 图片高(px)
		...
		// 验证码使用随机字体
		$ttfPath = dirname(__FILE__) . '/Verify/' . (C('VERIFY_TYPE') == 1 || C('VERIFY_TYPE') == 4 ? 'zhttfs' : 'ttfs') . '/';
		...
		$num5 = $num3 + 1;
			...
			if($full == 1){//加法
				$this->_codeVal = $num1 + $num2;
				$this->_codeSS = $num1.'+'.$num2.'=?';
			}elseif ($full == 2){//乘法
				$this->_codeVal = $num1 * $num2;
				$this->_codeSS = $num1.'*'.$num2.'=?';
			}elseif ($full == 3){//除法
				$this->_codeVal = $num5 / $num4;
				$this->_codeSS = $num5.'/'.$num4.'=?';
			}elseif ($full == 4){//减法
				$this->_codeVal = $num3 - $num6;
				$this->_codeSS = $num3.'-'.$num6.'=?';
			}

			$this->length = strlen($this->_codeSS);
		}
		...
		for ($i = 0; $i<$this->length; $i++) {
			...
			}elseif(C('VERIFY_TYPE') == 3){
				$code[$i] = $this->_codeSZ[mt_rand(0, 9)];
				$codeNX += mt_rand($this->fontSize*1.2, $this->fontSize*1.6);
				// 写一个验证码字符
				$this->useZh || imagettftext($this->_image, $this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize*1.6, $this->_color, $this->fontttf, $code[$i]);
			}else{
				$code[$i] = $this->_codeSS[$i];
				$codeNX += mt_rand($this->fontSize*1.2, $this->fontSize*1.6);
				// 写一个验证码字符
				$this->useZh || imagettftext($this->_image, $this->fontSize, mt_rand(-10, 10), $codeNX, $this->fontSize*1.6, $this->_color, $this->fontttf, $code[$i]);
			}
		}
		
		!empty($this->useZh) && imagettftext($this->_image, $this->fontSize, 0, ($this->imageL - $this->fontSize*$this->length*1.2)/3, $this->fontSize * 1.5, $this->_color, $this->fontttf, iconv("GB2312","UTF-8", join('', $code)));
		
		
		// 保存验证码
		$key = $this->authcode($this->seKey);
		if(C('VERIFY_TYPE') == 4){
			$code = $this->authcode($this->_codeVal);
		}else{
			$code = $this->authcode(strtoupper(implode('', $code)));
		}
		$session = array();
		if($id) {
			$session[$id]['aa'] = $this->fontttf;
