...
class UserController extends BlogController
{
    ...
    public function register($username = '',$nickname='', $password = '', $email = '', $verify = '', $type = 'start')
    {
        ...
        if (IS_POST) { //注册用户
            ...
            if (0 < $uid) { //注册成功
                ...
                $this->success('', U('Blog/User/step2'));
            } else { //注册失败，显示错误信息
    ...
    public function doCropAvatar($crop)
    {
        ...
        //显示成功消息
        $this->success($result['message'], U('Blog/User/step3'));
    }
    ...
    public function mi($username = '', $email = '', $verify = '')
    {
        ...
        if (IS_POST) { //登录验证
            ...
            //发送验证邮箱
            $url = 'http://' . $_SERVER['HTTP_HOST'] . U('Blog/User/reset?uid=' . $uid . '&verify=' . $verify);
    ...
    public function doReset($password, $repassword)
    {
        ...
        //显示成功消息
        $this->success('密码重置成功', U('Blog/User/login'));
    }
