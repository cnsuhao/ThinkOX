...
class RankController extends AdminController {
    ...
    public function index($page=1,$r=20){
        //读取数据
        $model = M('rank');
        $list = $model->page($page, $r)->order('create_time asc')->select();
        foreach($list as &$val){
    ...
    public function doUserAddRank($id=null,$uid,$reason,$is_show,$rank_id){
        ...
        if($is_Edit){
            ...
            if(!$result){
                ...
                $content1='由于'.$reason.'的原因，管理员给'.$u_name.'颁发了头衔：['.$rank['title'].']';//<img src="'.$logoUrl.'" title="'.$rank['title'].'" alt="'.$rank['title'].'">';
